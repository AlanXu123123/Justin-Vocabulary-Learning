#!/usr/bin/env node
// 05-translate.mjs
// Call enhanceWord per-word with a small concurrency pool so we never hit the
// 120s Cloud Function timeout. Builds ai-cache.json incrementally so re-runs
...
const SAVE_EVERY = 12; // save cache every N successes

function sleep(ms) { return new Promise(r => setTimeout(r, ms)); }

const byCat = JSON.parse(readFileSync(join(CLEAN, 'by-category.json'), 'utf8'));
...
console.log(`Total: ${wordList.length}, cached: ${wordList.length - todo.length}, to fetch: ${todo.length}`);
...
console.log(`Translate done in ${totalSecs}s. cache size: ${Object.keys(cache).length}, with Cn: ${withCn}, failed: ${failed}`);
